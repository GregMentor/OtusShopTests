...
Этот репозиторий можно использовать как стартовый шаблон для отработки unit- и integration-тестов на .NET 8: от чистой бизнес-логики до проверки HTTP-контрактов API.

> **Важно:** в README есть ссылка на теоретическую часть — см. раздел [`📚 Документация и примеры в коде`](#-документация-и-примеры-в-коде).
